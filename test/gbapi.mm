namespace gb {
    ...
    bendian struct VDMATransfer {
        writeonly u16 src;      // HDMA1 - New DMA Source, High      | HDMA2 - New DMA Source, Low (W)
	writeonly u16 dst;      // HDMA3 - New DMA Destination, High | HDMA4 - New DMA Destination, Low (W)
    ...
    struct CH1 {
            const u1;                 // NR10        - [unused] (R)
                  u3 sweepTime;       // NR10        - Chl 1 Sweep Time (R/W)
                  u1 sweepDir;        // NR10        - Ch1 Sweep Increase/Decrease (R/W)
	          u2 sweepSpeed;      // NR10        - Ch1 Number of sweep shift (R/W)
	          u2 duty;            // NR11        - Ch1 Wave Pattern Dute (R/W)
        writeonly u6 soundLengthData; // NR11        - Ch1 Sound length data (W)
                  u4 volume;          // NR12        - Ch1 Initial Volume of envelope (R/W)
                  u1 envelopeDir;     // NR12        - Ch1 Envelope Direction (R/W)
                  u3 envelopeSpeed;   // NR12        - Ch1 Initial Volume of envelope (R/W)
        writeonly u11 freq;           // NR13 / NR14 - Ch1 Frequency (W)
                  u1 counter;         // NR14        - Ch1 Counter (R/W)
                  u1 initial;         // NR14        - Ch1 Initial (W)
    }
    ...
    u4[32] waveTable =         *(0xFF30 as u4**);          // FF30 -> FF3F Wave Pattern RAM
